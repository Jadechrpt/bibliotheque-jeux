...
# === ANGLE DE REBONDISSEMENT DE LA BALLE ALÉATOIRE ===                                        
def direction_aleatoire(vitesse, vers_la_droite=True):
    angle_deg = random.uniform(20, 53)
    signe = random.choice(["-","+"]) #si "-" la balle part vers le haut, si "+" la balle part vers le bas
    if signe =="-":
            angle_deg=-angle_deg
    ...
    # Si la balle doit partir à droite, vx doit être positif
...
while ping:
        ...
        if mode == "adversaire":
                ...
                if (
                    ...
                    ball_speed_x, ball_speed_y = direction_aleatoire(BALL_SPEED, vers_la_droite=True)

                elif (
                    right_paddle_x < ball_x < right_paddle_x + paddle_width
                    and right_paddle_y < ball_y < right_paddle_y + paddle_height
                ):
                    ball_speed_x, ball_speed_y = direction_aleatoire(BALL_SPEED, vers_la_droite=False)
