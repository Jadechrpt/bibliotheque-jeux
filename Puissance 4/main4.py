...
hauteur = (NB_LIGNE + 1) * LARGEUR_CASE  # On laisse une ligne vide en haut
taille = (largeur, hauteur)
ecran = pygame.display.set_mode(taille)# On définit la fenêtre avec la taille calculée
pygame.display.set_caption("Jeu")
# Création du plateau avec des zéros partout (plateau vide)

#Variables necessaire pour trouver la police 
chemin_police = os.path.join("fonts", "PWHappyChristmas.ttf")
myfont = pygame.font.Font(chemin_police, 36)
...
def creer_plateau():
    plateau = []  # liste vide
    for i in range(NB_LIGNE -1, -1, -1):  # On parcourt les lignes
        ligne = [0] * NB_COLONNE  # Une ligne avec que des 0
        plateau.append(ligne)  # On ajoute les lignes au plateau
        
    return plateau  # retourne le plateau rempli de 0

def afficher_plateau(plateau):
    print("État actuel du plateau :")
    for ligne in range(NB_LIGNE - 1, -1, -1):  # Affiche de la dernière ligne à la première
        print(plateau[ligne])
...
def dessiner_plateau(plateau):
    ...
    # Dessiner les cercles blancs pour emplacements vides
...
def rectangle_fin_de_partie():
    ...
    texte_rect = texte_game_over.get_rect(center=(largeur // 2 , hauteur//10))  # Position Y à 50 pixels du haut
    ...
    image_background = pygame.transform.scale(image_background, (rectangle_largeur, rectangle_hauteur))  # Redimensionner image
...
class Button :
    ...
    def show(self):  # méthode pour afficher le bouton
        self.visible = True  
button_rejouer= Button("rejouer", 238, 450)#on crée le bouton rejouer
...
def afficher_scores():
    ...
    ecran.blit(score_text1, [(largeur // 3.5) - (score_text1.get_width() // 4), hauteur - LARGEUR_CASE-150 ])
    ecran.blit(score_text2, [(3 * largeur // 4.5) - (score_text2.get_width() // 2), hauteur - LARGEUR_CASE -150])
...
def victoire_ou_nul(joueur ):
    ...
    if joueur_gagnant(plateau, joueur): #verification de victoires
        ...
        ecran.blit(texte1, [145, 250])# Affiche texte_image à (50, 10) : 50 pixels à droite et 10 pixels en bas.              
        ecran.blit(image_noeud, (275, 95))
